...
def Gcam(model, tar_cate, image, img, device):
    ...
    with TemporaryGrad():
        ...
        mask = grad_rollout(input_tensor, int(tar_cate))
        np_img = np.array(image)[:, :, ::-1]
        mask = cv2.resize(mask, (np_img.shape[1], np_img.shape[0]))
        visualization = show_cam_on_image(np_img, mask)
...
def main():
    ...
    "alpha": 1,
    "input_size": 224,
    "mobilenet_name": mobile_vit_small,
    ...
    exper_mame = "lr_{}_{}_{}_{}_{}_{}".format(hyper_params["learning_rate"],
                         "Adam",
    ...
    # model
    model_name_set="mobile_vit_small"
    ...
    # create model
    net = hyper_params["mobilenet_name"](num_classes=hyper_params["num_classes"])
    # net = mobilenet_v3_large(num_classes=hyper_params["num_classes"], alpha=hyper_params["alpha"])
    # Initialize and train your model
    # model = TheModelClass()
    # train(model)
    best_net = hyper_params["mobilenet_name"](num_classes=hyper_params["num_classes"]).to(device)
    ...
    for epoch in range(hyper_params["epochs"]):
        ...
        # train
        running_loss = one_epoch_train(net,
                                        train_loader, 
                                        optimizer,
                                        device, 
                                        loss_function,
        ...
        # validate
        val_accurate = one_epoch_valid(experiment, net, validate_loader, device, epoch, hyper_params, val_num)
    ...
    # log_model(experiment, net, model_name="mobilenet_v3_large")
        ...
        if val_accurate > best_acc:
            ...
            best_net.eval()
            # test 
            test_best(experiment, cla_dict, testdate_loader, best_net, device, epoch, hyper_params, test_num, reback_img)
